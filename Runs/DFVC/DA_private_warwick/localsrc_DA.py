...
class WSIRandomBigchunkLoader(BigChunkLoader):
    ...
    def extract_bigchunk(self, last_message_fromroot):
        ...
        try:
            ...
            print("failed for patient {}".format(self.patient))
...
class Global_Discriminator(nn.Module):
    def __init__(self, in_feature_dim):
        ...
        self.dim = in_feature_dim  # feat layer          256*H*W for vgg16
...
class Local_Discriminator(nn.Module):
    def __init__(self, in_feature_dim):
        ...
        self.dc_ip1 = nn.Linear(in_feature_dim, 1024)
...
class Pipeline_DA(BoVWPipeline):
    def __init__(self, num_classes, num_visualwords, device_stg3, *args, **kwargs):
        ...
        #call on super
        super(Pipeline_DA, self).__init__(*args, **kwargs)
        self.global_DA = Global_Discriminator(2048)
        self.local_DA = Global_Discriminator(200)
...
#     def forward(self, src_x, tgt_x, tensor_list_assignmentindices):
#         '''
#         Inputs:
...
#         '''
#         src_output = self.module_stg1[0](src_x)
#         tgt_output = self.module_stg1[0](tgt_x)
        
#         src_DA_output = self.global_DA(src_output)
#         tgt_DA_output = self.global_DA(tgt_output)
#         #DA part
# #         print(src_output.shape)
# #         print(tgt_output.shape)
        
        
#         output = self.module_stg1[1](src_output)
...
#         return output, src_DA_output, tgt_DA_output
        
    def forward(self, src_x, tgt_x, src_tensor_list_assignmentindices, tgt_tensor_list_assignmentindices):
        '''
        Inputs:
            - x: batch of images, a tensor of shape, e.g., [10x3x224x224].
            - tensor_list_assignmentindices:
        ...
        src_output = self.module_stg1[1](src_output)
        tgt_output = self.module_stg1[1](tgt_output)
        
        _, src_output = self.module_stg2(src_output)
        _, tgt_output = self.module_stg2(tgt_output)
       
        src_output = self.module_stg3(src_output, src_tensor_list_assignmentindices)
        tgt_output = self.module_stg3(tgt_output, tgt_tensor_list_assignmentindices)
        
#         print(src_output.shape)
#         print(tgt_output.shape)
        
        src_local_DA_output = self.local_DA(src_output)
        tgt_local_DA_output = self.local_DA(tgt_output)
        
        output = self.module_stg4(src_output)
        return output, src_DA_output, tgt_DA_output, src_local_DA_output, tgt_local_DA_output
